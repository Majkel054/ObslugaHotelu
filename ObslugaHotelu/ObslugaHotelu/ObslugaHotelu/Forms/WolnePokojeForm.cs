...
namespace ObslugaHotelu.Forms
{
    public class WolnePokojeForm : Form
    {
        ...
        private void WczytajWolnePokoje()
        {
            ...
            dgvWolnePokoje.Columns.Add("LiczbaMiejsc", "Liczba miejsc");
            dgvWolnePokoje.Columns.Add("LozkaPojedyncze", "Łóżka pojedyncze");
            dgvWolnePokoje.Columns.Add("LozkaPodwojne", "Łóżka podwójne");
            ...
            foreach (var linia in wszystkiePokoje)
            {
                ...
                if (danePokoju.Length >= 4)
                {
                    string numerPokoju = danePokoju[0];
                    string liczbaMiejsc = danePokoju[1];
                    string lozkaPoj = danePokoju[2];
                    string lozkaPodw = danePokoju[3];

                    bool czyZajety = zajetePokoje.Contains(numerPokoju);

                    if (!czyZajety)
                    {
                        dgvWolnePokoje.Rows.Add(numerPokoju, liczbaMiejsc, lozkaPoj, lozkaPodw);
                    }
                }
            }
        }
